...
public class SpotCommand {
    ...
    private static final SuggestionProvider<FabricClientCommandSource> TELEPORT_SUGGESTIONS = (context, builder) -> {
        ...
        for (String target : SPECIAL_TARGETS) {
            if (target.startsWith(builder.getRemaining().toLowerCase())) {
    ...
    private static int addSpot(String name) {
        ...
        if (dataManager.addSpot(name, player.getX(), player.getY(), player.getZ(),
                getCurrentDimension(), getCurrentWorldName())) {
    ...
    private static int debugUserData() {
        ...
        if (!client.isInSingleplayer()) {
            sendFeedback("[SpottedDog] 此命令仅在单人模式下可用");
            return Command.SINGLE_SUCCESS;
        }

        IntegratedServer server = client.getServer();
        if (server == null) {
            sendFeedback("[SpottedDog] server is null");
            return Command.SINGLE_SUCCESS;
        }
        ...
        if (list != null) {
            sendFeedback("[SpottedDog] getPlayerList returned: " + list.getClass().getName());

            for (ServerPlayerEntity playerEntity :  list) {
                if (playerEntity.getRespawn() != null) {
                    sendFeedback("  - " + playerEntity.getRespawn().respawnData().getPos());
                }
                sendFeedback("  - " + playerEntity.getLastDeathPos());
            }
        } else {
            sendFeedback("[SpottedDog] getPlayerList returned null");
        }
